...
const ChartPanel = ({ isOpen, onClose, messages, onSendMessage, currentUser }) => {
    ...
    return (
        <aside className="w-full sm:w-80 h-full bg-white border-l border-slate-200 flex flex-col z-30 shadow-2xl animate-in slide-in-from-right duration-200">
            ...
            <div className="flex-1 p-4 overflow-y-quto space-y-4">
                {messages.length === 0 ? (
                    ...
                    messages.map((msg, index) => {
                        ...
                        return (
                            <div key={msg.id || index} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                                ...
                                <div className={`px-3.5 py-2.5 rounded-2xl max-w-[85%] text-sm leading-relaxed shadow-xs ${isMe ? "bg-primary text-white rounded-tr-none font-medium" : "bg-slate-100 text-slate-800 rounded-tl-none border border-slate-200 font-medium"}`}>
                                    {msg.text}
                                </div>
                            </div>
                        )
            ...
            <form onSubmit={handleSubmit} className="p-3 border-t border-slate-200 bg-slate-200 bg-slate-50 flex items-center gap-2">
                <input type="text"
                ...
                <button type="submit" disabled={!text.trim()} className="p-2.5 rounded-xl bg-primary hover:bg-primary-hover text-white disabled:opacity-40 transition-all cursor-pointer shadow-xs">
