import { CrownIcon, MicOffIcon, MicIcon, VideoIcon, VideoOffIcon, XIcon } from "lucide-react";

const PaticipantList = ({ isOpen, onClose, localUser, localAudio, localVideo, remoteUsers, meetingHostId }) => {

    if (!isOpen) return null;

    const allParticioants = [{
        socketId: "local",
        userId: localUser?.id,
    ...
    return (
        <aside className="w-full sm:w-80 h-full bg-white border-l border-slate-200 flex flex-col z-30 shadow-2xl animate-in slide-in-from-right duration-200">
            <div className="p-4 border-b border-slate-200 flex items-center justify-between">
                <h3 className="font-medium text-slate-900 text-base flex items-center gap-2">Paticipants ({allParticioants.length})</h3>
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-3">
                {allParticioants.map((p) => {
                    const isHost = meetingHostId && p.userId === meetingHostId

                    return (
                        <div key={p.socketId} className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-200 shadow-xs">
